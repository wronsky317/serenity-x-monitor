...
7. 重要性不以是否含证券代码为前提。Serenity 帖子中的人物/基金动态、FOMC 利率决议、监管政策、重大宏观事件、产业拐点，即使没有具体标的，也必须在“今日核心总结”和“重点内容解读”中保留；不要把人物信息压缩成纯价格波动，也不要把宏观内容硬映射成荐股。
8. 不给投资买卖建议，不使用 Markdown 表格，不输出 raw JSON，不贴长篇原文。

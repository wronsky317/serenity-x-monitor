...
Serenity's framework is centered on the physical bottlenecks of the AI supercycle. The core idea is that AI value does not stop at GPU winners. It propagates through photonics, CPO, lasers, memory/HBM, data-center power, advanced packaging, PCB, and later physical AI/robotics.

The two highest-conviction long-term themes are:

1. Photonics / CPO / CW lasers.
2. Memory / HBM / DRAM/NAND.

Secondary but important themes:
...
## Theme 1: Photonics / CPO / CW Laser

### Core Companies

- `SIVE.ST` / `SIVE`: highest-conviction core asset. Serenity repeatedly frames Sivers as a merchant CW-laser chokepoint across 1.6T, pluggables, NPO, CPO scale-out and scale-up, Ayar, Jabil, GFS, AMD, MRVL, Apple/consumer photonics, and robotics sensing. The cross-architecture exposure matters more than any single CPO launch date.
- Recent catalyst update: Sivers 700m SEK institutional financing, described as close to market price and multiple-times oversubscribed, materially de-risks InP lasers / optical amplifiers expansion if proceeds and production ramp execute as expected.
- `LITE`: large-cap photonics reference case and peer. Often used as the prior rerating template.
- `AAOI`: high-beta optical transceiver / AI networking optics beneficiary.
- `COHR`: larger optical-components supplier and likely CPO-adjacent beneficiary.
...
- `MU`: central US-listed memory expression.
- `000660.KO` SK Hynix: HBM and AI memory leader exposure.
...
- 2026 to 2028: structural AI server and HBM demand.
- 2027 onward: physical AI, humanoid robotics, and edge/on-device AI may add another memory-demand leg.
...
- Micron earnings, guidance, LTAs, and pricing commentary.
- Micron customer LTAs, including the reported Qualcomm relationship, and whether contract terms support volume and pricing visibility rather than merely reserving supply.
- SK Hynix and Samsung HBM capacity, margin, and capex.
- HBM4 transition and advanced packaging constraints.
...
- AI servers increase memory content and bandwidth requirements.
...
- HBM supply catches up faster than expected.
- Pricing weakens before capex payback.
...
- `AOSL`: US power semiconductor exposure.
- `POWI`: US power semiconductor exposure.
- `COPX`, `BHP`: copper and upstream material exposure.
- `VRT`, `ETN`, `POWL`, `VICR`: data-center power, electrical equipment, and power architecture.
- `FLNC`: energy storage / hyperscaler battery-storage optionality.
- `MP` and other US rare-earth / magnet-chain proxies: watch-only exposure to the AI cold-war and robotics supply-chain localization frame. Recent references were thematic or parser-derived rather than full single-name theses.
- China read-through names: Yangjie Technology, China Resources Micro, Silan Microelectronics, NCE Power.

### Possible Cycle

- 2026: Chinese power semiconductor price hikes validate demand tightness.
- 2026 to 2027: AI data centers pull forward 800V DC and power-conversion architecture.
- 2027 to 2028: power bottlenecks may become a central constraint for AI buildouts.
- 2026 to 2028: AI cold-war framing can keep hyperscaler capex politically and strategically supported, while rare earths, magnets, EUV/EDA, Japanese vacuum/furnace equipment, and US reshoring become chokepoint watch areas.

### Watchpoints

- Continued power semiconductor price hikes.
- AI data-center adoption of 800V DC architectures.
- Hyperscaler power contracts, grid interconnection constraints, and storage deals.
- Copper/tin price and supply-chain tightness.
- Whether Chinese price hikes are demand-driven or temporary cost/pass-through effects.
- US policy actions around AI chips, rare earths, magnets, ASML/EUV, EDA, and allied semiconductor equipment suppliers.
- OpenAI / US government stake or AI infrastructure backstop discussions; currently hypothetical and not a confirmed policy thesis.
...
- Power architecture shifts do not benefit listed power semi names.
...
- `NBIS`: preferred neocloud winner in Serenity's framework.
...
- `NVDA`: ecosystem anchor.
- `IREN`: mostly negative watch / avoid or short candidate in Serenity's posts due to dilution, governance, ATM, and questionable AI partnership narratives. A later Australia sovereign DC / Anthropic-capacity post mentions `IREN` and `SHAZ` as possible regional capacity beneficiaries, but this should not override the governance / dilution caution without contracts and financing details.
- `SHAZ`: watch-only Australia sovereign DC / neocloud mention from the Anthropic-capacity narrative; needs customer, power, financing, and up-time evidence.

### Possible Cycle

- 2026: market distinguishes real AI compute platforms from miner-to-AI narratives.
...
- Compute supply becomes abundant.
- Neocloud utilization or margins disappoint.
- Financing becomes expensive or dilutive.
- Hyperscalers internalize demand and reduce third-party leasing.
...
- Schaeffler / `SHA0`, Nabtesco `6268`, Sanhua: auto-parts / actuator / joint / power-electronics watchlist. Codes and market mappings require manual confirmation where upstream ticker validation failed.
- `SIVE.ST`: sensing, LiDAR, Boston Dynamics Atlas, Nvidia Hyperion, and consumer/robotics laser optionality.
- `AEVA`: sensing / LiDAR pathway.
- `AAPL`, `GOOGL`: consumer photonics / wearable sensing option.
- `AMZN`, `JD`, `GM`, `MELI`, `DASH`, `UBER`: labor-dense platforms or industrial users where robotics can affect delivery, warehouse, maintenance, or opex economics. Some are Serenity extrapolations, not confirmed deployment-equivalent theses.
- `NVDA`: robotics stack / factory robotics / physical AI ecosystem enabler.
- `SYM`, `ROK`, `TER`, `CGNX`: warehouse automation, industrial automation, test, and machine-vision watchlist; evidence strength varies by post.
- `RHI`: possible labor-replacement negative proxy from structured output, not a direct core thesis.

### Possible Cycle

- H2 2026 to 2027: pure robotics / humanoid IPO season and Agility / CCXI listing path are major catalysts.
- 2026 to 2027: physical AI narrative broadens from Tesla and Nvidia ecosystem to Agility, Unitree, LeaderDrive, Harmonic Drive, US supply-chain localization, and large-employer automation.
- 2027 onward: robots may become a real memory, sensing, actuator, reducer, edge-AI, and labor-substitution demand driver.
- 2027 onward: auto-parts companies such as Schaeffler / Nabtesco / Sanhua may receive humanoid or AI Auto catalysts if robot BOM share becomes revenue-visible.
- Later-cycle option: consumer photonics and wearable sensing if Apple/Google/Samsung adoption is validated.
...
- Nvidia Hyperion / robotics ecosystem supplier confirmations.
- Boston Dynamics or industrial robotics sensing supply-chain evidence.
- Boston Dynamics / Atlas actuator output, installed capacity, yield, ASP, and supplier qualification. These production inputs should carry more weight than linear annual whole-robot forecasts.
...
- Physical AI can pull through memory, sensors, optical components, actuators, and edge compute.
- Serenity treats robotics as a later wave that reinforces memory and photonics.
- Recent posts frame robotics as a national-security and labor-productivity thesis, not just a Tesla product cycle.
- The key transition is from demo-stage to pre-scale / scaled deployment: warehouses, delivery, factories, data-center maintenance, and general labor substitution.
- Robot-company partnerships with automotive groups may provide manufacturing and deployment leverage, but binding customer relationships, commercial orders, and repeatable multi-customer sales matter more than affiliation alone.
- US humanoid producers remain constrained by their current manufacturing-speed and supply-chain gap versus China; local actuator and component capacity is a thesis prerequisite, not an assumed advantage.
- Public-market exposure is fragmented; many best assets remain private, making component suppliers and SPAC / listed proxies higher risk.

### Invalidation Risks

- Humanoid adoption stays demo-stage.
- Actuator capacity, output, yield, ASP, or supplier qualification fails to support the expected production ramp; personal 2028-2030 unit estimates should not override observed operating data.
- Component suppliers do not capture value.
- Memory and sensing content per robot is lower than expected.
- End products fail on cost, safety, or reliability.
- CCXI / Agility listing is delayed, cancelled, repriced, or fails to bring durable public-market exposure.
- US-made supply-chain claims prove uneconomic versus China suppliers.
...
- 2026: AI PCB and advanced packaging demand continues to appear in supply-chain updates.
...
- HBM4 and advanced packaging capex.
- CPO inspection and packaging/test order flow.
- TSMC PIC expansion, Largan Precision FAU production, and whether FAU / CPO integration can overcome the near-term substitution difficulty around Corning glass-bridge approaches.
...
- AI server complexity increases packaging and PCB value.
...
- `RDDT`: platform rerating / high-growth high-margin thesis.
- `IBKR`: favored over `HOOD` for global market access and product breadth.
- `HOOD`: retail/crypto beta, but less favored structurally than IBKR in some posts.
- `COIN`, `CRCL`: crypto-native finance.
- `RKLB`: space / defense narrative, high beta.
- `ASTS` / Rakuten / `SPCX` Starlink: LEO satellite competition watch, currently based on a failed-but-readable item and secondary-source references. Keep as satellite / defense / space observation, not a core AI supply-chain thesis.
- `MRVL`: already part of photonics / AI networking, but some recent "next $1T company" references were sentiment reminders without fresh evidence.
- `TLT`, `IEF`: rate-view expression, not a core industrial asset.

### Role In Portfolio Map

These are not the center of Serenity's long-term AI supply-chain framework. Treat as satellites, tactical expressions, or separate theses.

## Negative / Caution List

- `IREN`: repeated negative view; dilution, ATM, and questionable AI/Anthropic narrative.
- `IREN`: additionally flagged for founder stock-based compensation and active ATM risk. Later Australia / Anthropic capacity mentions are watch-only until contracts and financing are verified.
- `BOT`: NAV premium and dilution risk.
- Miner-to-AI pivots generally: skepticism unless financing and customer quality are clear.
...
### P3: Optionality / Watchlist

- `688017.SHG` LeaderDrive / 绿的谐波
...
When updating this file, check:

1. Did Serenity explicitly reinforce or weaken a theme?
2. Was a new company mentioned as a direct beneficiary, customer, supplier, or negative example?
3. Did a thesis move from optionality to confirmed catalyst?
4. Did timing change, especially around 2026 H2, 2027, or 2028 ramps?
5. Did a source event invalidate supply-chain mapping?
6. Are there new non-US tickers or Chinese/Japanese/Korean names that should be added?
7. Are any names no longer core and should be moved to watch-only or caution?

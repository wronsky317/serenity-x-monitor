...
Delivery requirement: every scheduled Hermes run must create a concise Chinese daily report at `reports/latest_summary.md`; Hermes cron delivers the final suite response to Feishu group `祖国的花朵` via its local delivery config. Manual sends should use `FEISHU_CHAT_ID`. If no new posts are found, send a short `今日无新增/抓取结果` report rather than staying silent. If sources fail, send the failure reason and the last processed post/time.
